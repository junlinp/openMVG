...
ceres::CostFunction* IntrinsicsToCostFunction(
    openMVG::cameras::IntrinsicBase* intrinsic, const openMVG::Vec2& observation,
    const double weight) {
...
void CPUProcessor::ZOptimization() {
  camera_consensus_params_ = ZUpdate(camera_params_, camera_params_multipler_, rho_);
  pose_consensus_params_ = ZUpdate(pose_consensus_params_, pose_params_multipler_, rho_);
  structure_consensus_params_ = ZUpdate(structure_consensus_params_, structure_params_multipler_, rho_);
}

void CPUProcessor::YUpdate() {
  for(size_t i = 0; i < camera_params_multipler_.size(); i++) {
    camera_params_multipler_[i] += rho_ * (camera_params_[i] - camera_consensus_params_[i]);
  }

  for(size_t i = 0; i < pose_params_multipler_.size(); i++) {
    pose_params_multipler_[i] += rho_ * (pose_params_[i] - pose_consensus_params_[i]);
  }

  for (size_t i = 0; i < structure_params_multipler_.size(); i++) {
    structure_params_multipler_[i] += rho_ * (structure_params_[i] - structure_consensus_params_[i]);
  }
}
