...
TEST(BUNDLE_ADJUSTMENT_ADMM, Optimize) {
  ...
  std::vector<double> pose_vector = {
      0.0, 0.0, 0.0, pose.center()(0), pose.center()(1), pose.center()(2)};
  ...
  std::shared_ptr<openMVG::sfm::Processor> processor =
      std::make_shared<openMVG::sfm::CPUProcessor>(intrinsic->getType(),
                                                   intrinsic->getParams(),
                                                   pose_vector, X, x, options);
...
std::vector<std::shared_ptr<openMVG::sfm::Processor>> CreateScene() {
  ...
  std::shared_ptr<openMVG::sfm::Processor> processor =
      std::make_shared<openMVG::sfm::CPUProcessor>(
          intrinsic1->getType(), intrinsic1->getParams(), pose_vector1, X1, x1,
          options);

  std::shared_ptr<openMVG::sfm::Processor> processor2 =
      std::make_shared<openMVG::sfm::CPUProcessor>(
          intrinsic2->getType(), intrinsic2->getParams(), pose_vector2, X2, x2,
          options);
